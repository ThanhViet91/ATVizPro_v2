...
//    System.out.println("thanh ====> BaseEncoder ---48 -- "+ mCamID);
  ...
  private void initCodec() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {   //vào đây
      createAsyncCallback();
      codec.setCallback(callback, handler);
      codec.start();
//      System.out.println("thanh ===> vao day BaseEncoder  --- Name Codec " + thanhtest );
    } else {
  ...
  private void createAsyncCallback() {
    callback = new MediaCodec.Callback() {
      ...
      public void onInputBufferAvailable(@NonNull MediaCodec mediaCodec, int inBufferIndex) {
        try {
//          System.out.println("thanh ====> "+ mCamID +" BaseEncoder -- 225");
